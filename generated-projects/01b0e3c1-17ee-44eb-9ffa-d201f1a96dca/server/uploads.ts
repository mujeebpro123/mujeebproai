...
uploadsRouter.post("/api/scan-menu", async (req, res) => {
  try {
    ...
    if (!openai) {
      return res.status(503).json({ error: "AI menu scanning is not configured in this preview." });
    }
...
uploadsRouter.post("/api/ai/generate-image", async (req, res) => {
  try {
    ...
    if (!openai) {
      return res.status(503).json({ error: "AI image generation is not configured in this preview." });
    }
