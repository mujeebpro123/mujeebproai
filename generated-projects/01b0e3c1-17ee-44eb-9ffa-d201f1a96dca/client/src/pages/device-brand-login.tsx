...
export default function DeviceBrandLogin() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/device-brands/login", {
        method: "POST",
      ...
      localStorage.setItem("deviceBrandId", brand.id);
      localStorage.setItem("deviceBrandSlug", brand.slug);
      localStorage.setItem("deviceBrandName", brand.name);
      setLocation("/device-brand-dashboard");
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white/5 border-white/10 backdrop-blur-xl">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-2xl font-bold text-white">Device Brand Admin</CardTitle>
