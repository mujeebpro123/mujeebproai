...
import { ChefHat, Lock, Loader2, Store } from "lucide-react";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/branch-login-by-name", {
        ...
        body: JSON.stringify({ restaurantName: loginRestaurantName, password: password.trim(), role: "kitchen" }),
      });
      ...
      localStorage.setItem("kitchenBranchSlug", data.restaurant.slug);
      localStorage.setItem("kitchenBranchName", data.restaurant.name);
      
      toast({ title: "Kitchen Ready", description: `Logged into ${data.restaurant.name}` });
      setLocation(`/kitchen/${data.restaurant.slug}`);
    } catch (error) {
      toast({ 
        title: "Login failed", 
  ...
  return (
    <>
      <PwaSplashScreen
        appName="App Kitchen"
        accentColor="#f97316"
        iconComponent={<ChefHat className="h-12 w-12" />}
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-orange-900/20 to-slate-900 flex items-center justify-center p-4">
        ...
        <Card className="w-full max-w-md border-slate-700 bg-slate-800/50 backdrop-blur">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl text-white">Kitchen Display Login</CardTitle>
            <CardDescription className="text-slate-400">
              Enter your branch details to access the kitchen
            </CardDescription>
          </CardHeader>
          <CardContent>
