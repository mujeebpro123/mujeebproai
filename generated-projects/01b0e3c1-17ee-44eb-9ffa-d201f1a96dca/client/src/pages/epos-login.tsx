...
import { CreditCard, Lock, Loader2, Store } from "lucide-react";
...
export default function EposLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/branch-login-by-name", {
        ...
        body: JSON.stringify({ restaurantName: loginRestaurantName, password: password.trim(), role: "epos" }),
      });
      ...
      localStorage.setItem("eposBranchSlug", data.restaurant.slug);
      localStorage.setItem("eposBranchName", data.restaurant.name);
      
      toast({ title: "EPOS Ready", description: `Logged into ${data.restaurant.name}` });
      setLocation(`/epos/${data.restaurant.slug}`);
    } catch (error) {
      toast({ 
        title: "Login failed", 
  ...
  return (
    <>
      <PwaSplashScreen
        appName="App Epos"
        accentColor="#3b82f6"
        iconComponent={<CreditCard className="h-12 w-12" />}
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900/20 to-slate-900 flex items-center justify-center p-4">
        ...
        <Card className="w-full max-w-md border-slate-700 bg-slate-800/50 backdrop-blur">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl text-white">EPOS Login</CardTitle>
            <CardDescription className="text-slate-400">
              Enter your branch details to access EPOS
            </CardDescription>
          </CardHeader>
          <CardContent>
