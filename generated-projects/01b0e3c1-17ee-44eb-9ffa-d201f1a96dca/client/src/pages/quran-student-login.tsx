...
export default function QuranStudentLogin() {
  ...
  const login = useMutation({
    mutationFn: () => fetch("/api/quran/student-login", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username, password }),
    }).then(r => { if (!r.ok) throw new Error("Invalid credentials"); return r.json(); }),
    onSuccess: (data: any) => {
      localStorage.setItem("quranStudentId", data.student.id);
      localStorage.setItem("quranStudentName", data.student.name);
      localStorage.setItem("quranStudentAcademyId", data.student.academyId);
      localStorage.setItem("quranStudentAcademyName", data.academy?.name || "Quran Academy");
      setLocation("/quran-student-dashboard");
    },
    ...
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center" style={{ background: "linear-gradient(135deg, #0a2e1a 0%, #0d3d23 25%, #0a2e1a 50%, #071e12 100%)" }}>
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input value={username} onChange={e => setUsername(e.target.value)} placeholder="Enter your username"
                className="bg-white/5 border-emerald-500/20 text-white placeholder:text-emerald-300/30 mt-1" data-testid="input-student-login-username"
                onKeyDown={e => e.key === "Enter" && login.mutate()} />
            </div>
            <div>
              <Label className="text-emerald-300/70 text-sm">Password</Label>
              <Input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password"
                className="bg-white/5 border-emerald-500/20 text-white placeholder:text-emerald-300/30 mt-1" data-testid="input-student-login-password"
                onKeyDown={e => e.key === "Enter" && login.mutate()} />
            </div>
            <Button onClick={() => login.mutate()} disabled={login.isPending} className="w-full bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white h-12 text-lg shadow-lg shadow-emerald-500/20" data-testid="button-student-login">
              <LogIn className="h-5 w-5 mr-2" /> {login.isPending ? "Logging in..." : "Login"}
            </Button>
          </div>
        </div>

        <div className="mt-8 bg-white/5 backdrop-blur-sm border border-emerald-500/10 rounded-xl p-5 text-center">
        ...
        <div className="text-center mt-4">
          <button onClick={() => setLocation("/quran-academy-login")} className="text-emerald-400/60 text-sm hover:text-emerald-400 transition-colors underline underline-offset-4" data-testid="link-academy-login">
            Academy Admin? Login here
          </button>
        </div>
